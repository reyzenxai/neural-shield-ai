/**
 * Shared scan request schemas. These are the same rules the backend enforces, so
 * a client can validate input before sending and get the identical result. The
 * backend keeps its own copy in backend/src/schemas for now; it will re-export
 * from here once the package build pipeline is in place (see docs/migration-guide.md).
 */
